...
from chirrp.rag_pipeline.key_utils import resolve_gemini_api_key
...
# How to chunk: "auto" (try Gemini, fall back to deterministic on failure/quota),
# "deterministic" (never call Gemini — robust on free-tier keys), or "llm"
# (Gemini only, raise on failure). Free-tier generateContent is ~20 req/day, so
# large corpora exhaust it fast; "auto" keeps ingestion working regardless.
RAG_CHUNK_MODE = os.environ.get("RAG_CHUNK_MODE", "auto").lower()

# Nebraska statute / regulation section markers, e.g. "46-701" or "Section 2-1".
_SECTION_RE = re.compile(r"\b\d{1,3}-\d{1,4}\b")
...
def _call_gemini_json(payload: bytes) -> Dict[str, Any]:
    ...
    if not api_key:
        raise RuntimeError("No Gemini API key found in gemini_api.txt or GEMINI_API_KEY")
...
def _deterministic_chunks(window_text: str, source_name: str,
                          max_chunk_chars: int) -> List[Dict[str, str]]:
    """LLM-free chunking: pack paragraphs into <= max_chunk_chars pieces and
    label each with a detected statute section number (or the first line).

    Used as the fallback when the Gemini chunker is unavailable (e.g. free-tier
    quota), so RAG ingestion still produces good retrieval chunks.
    """
    pieces = _split_large_chunk(window_text, max_chunk_chars=max_chunk_chars)
    cleaned: List[Dict[str, str]] = []
    for p in pieces:
        m = _SECTION_RE.search(p[:200])
        if m:
            heading = f"Section {m.group(0)}"
        else:
            first_line = next((ln.strip() for ln in p.splitlines() if ln.strip()), "")
            heading = (first_line[:80] or source_name).strip()
        cleaned.append({
            "heading": heading,
...
def logical_chunk_text_gemini(
    ...
    3. Chunk each window logically (Gemini, with a deterministic fallback)
    ...
    for w_idx, window_text in enumerate(windows):
        gemini_chunks: List[Dict[str, str]]
        if RAG_CHUNK_MODE == "deterministic":
            gemini_chunks = _deterministic_chunks(window_text, source_name, max_chunk_chars)
        else:
            try:
                prompt = _build_chunk_prompt(
                    window_text, source_name=f"{source_name} window {w_idx + 1}")
                payload = _make_generation_payload(prompt)
                raw = _call_gemini_json(payload)
                gemini_chunks = _validate_chunks(raw)
            except Exception as e:
                if RAG_CHUNK_MODE == "llm":
                    raise
                print(f"  [chunk] Gemini chunker failed ({str(e)[:120]}); "
                      f"falling back to deterministic chunking for window {w_idx + 1}")
                gemini_chunks = _deterministic_chunks(window_text, source_name, max_chunk_chars)
